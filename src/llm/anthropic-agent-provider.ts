...
/*
 * Re-export the shared structured-JSON helpers — they were MOVED VERBATIM to
 * `./structured-json` (pure, offline, provider-agnostic) so the OpenAI-compatible
 * provider can reuse them without pulling the Agent SDK. They remain part of this
 * module's surface so existing importers/tests that pull `AgentProviderError`,
 * `buildPromptModePrompt`, `extractJsonObject`, and `validateAgainstSchema` from
 * the Anthropic provider keep working unchanged.
 */
export { AgentProviderError, buildPromptModePrompt, extractJsonObject, validateAgainstSchema };
